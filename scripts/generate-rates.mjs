...
/**
 * What this table is, so the service can tell that it has changed.
 *
 * A group whose history was brought in against an older table has drops in it that could not be
 * scored then and can be now. Nothing would ever go back for them: a finished import leaves a mark
 * saying how far through the channel it read, and later imports start from there. Stamping the data
 * itself means the service can notice it has learned something since, and read the channel again once
 * rather than every time.
 *
 * A hash of the content, so it changes when the rates change and not when this script is edited.
 */
...
console.log(`${Object.keys(table).length} sources, ${kept} drops -> ${OUT} (version ${version})`);
if (unnamed > 0)
{
